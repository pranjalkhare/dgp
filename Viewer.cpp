...
Shape * Viewer::root = NULL;
std::vector<Renderable*>  Viewer::renderables;
...
void
Viewer::setRenderables(std::vector<Renderable*> const & renderables_)
{
...
Viewer::draw()
{
  ...
  if (root)
  {
    ...
    render_system->setMatrixMode(Graphics::RenderSystem::MatrixMode::PROJECTION); render_system->pushMatrix();
      ...
      Real scale = root->bbox.getExtent().length();

      for (size_t i = 0; i < renderables.size(); ++i)
        renderables[i]->draw(*render_system);
